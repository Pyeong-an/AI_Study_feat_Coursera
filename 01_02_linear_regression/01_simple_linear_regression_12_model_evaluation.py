...
y_test_ = regressor.predict(X_test.reshape(-1,1)) # 테스트용 입력 데이터를 2차원 배열로

print("Mean absolute error: %.2f" % mean_absolute_error(y_test, y_test_)) # MAE 평균절대오차
print("Mean squared error: %.2f" % mean_squared_error(y_test, y_test_)) # MSE 평균제곱오차
print("Root mean squared error: %.2f" % np.sqrt(mean_squared_error(y_test, y_test_))) # RMSE 평균제곱근오차
print("R2-score: %.2f" % r2_score(y_test, y_test_)) # R2 결정계수 점수