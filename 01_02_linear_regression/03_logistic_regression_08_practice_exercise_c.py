...
churn_df = churn_df[['tenure', 'age', 'address', 'income', 'ed', 'employ', 'equip', 'callcard', 'wireless', 'churn']] # 필요열만 선택
churn_df['churn'] = churn_df['churn'].astype('int') # 값들을 int로 변환(정수)

X = np.asarray(churn_df[['tenure', 'age', 'address', 'income', 'ed', 'employ', 'equip', 'callcard', 'wireless']]) # 필요열 선택 후 array로 변환
